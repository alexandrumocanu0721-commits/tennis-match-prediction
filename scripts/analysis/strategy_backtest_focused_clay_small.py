...
EQUITY_OUTPUT = PROJECT_ROOT / "data" / "processed" / "strategy_backtest_focused_clay_small_equity.csv"
MONTHLY_OUTPUT = PROJECT_ROOT / "data" / "processed" / "strategy_backtest_focused_clay_small_monthly.csv"
...
def main() -> None:
    ...
    eligible = df.loc[
        df["surface"].eq(TARGET_SURFACE)
        & df["odds"].lt(MAX_ODDS)
        & df["edge"].ge(EDGE_MIN)
        & df["edge"].lt(EDGE_MAX)
    ].copy()
    eligible = eligible.sort_values(["date", "source_row_idx"], kind="stable").reset_index(drop=True)
    eligible["edge_tier"] = "Small"

    if eligible.empty:
        print("=== Focused Strategy Backtest: Challenger Clay Small Edge ===")
    ...
    print("=== Focused Strategy Backtest: Challenger Clay Small Edge ===")
    print(
        "Rules: Challenger only, Clay only, odds < 1.50, "
        "calibrated edge 0.03-0.05, flat 1.00% stake, 10.00% daily cap"
    )
    print(
        "Date range: "
